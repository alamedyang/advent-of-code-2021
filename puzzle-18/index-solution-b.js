...
// doHomework(THINGY) for each combination of lines:

let finalMagnitudes = [];

lines.forEach(function (line1, index1) {
	lines.forEach(function (line2, index2) {
		if (index1 != index2) {
			const sum = addThings(line1, line2);
			const homework = doHomework(sum);
			const magnitude = getFullMagnitude(homework);
			finalMagnitudes.push(magnitude);
		}
	})
});

const finalFinalFinal = Math.max(...finalMagnitudes);

console.log(finalFinalFinal);
console.log(finalMagnitudes);
